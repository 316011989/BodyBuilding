...
public class FansViewModel extends AndroidViewModel {
    ...
    private LiveData<FansData> mLiveObservableData;
    //UI使用可观察的数据 ObservableField是一个包装类，可使UI变更
    public ObservableField<FansData> uiObservableData = new ObservableField<>();
    //后台调度，可关闭
    private final CompositeDisposable mDisposable = new CompositeDisposable();
    //动态数据，请求获取，可使mLiveObservableData变更
    private MutableLiveData<FansData> applyData = new MutableLiveData<>();
    //动态boolean值，根据接口进度变化
    ...
    private void LoadData() {
        ...
        GankDataRepository.getFansDataRepository(count, index)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<FansData>() {
                    @Override
                    ...
                    @Override
                    public void onNext(FansData value) {
                        LogUtils.e(value);
     ...
     * @return
     */
    public LiveData<FansData> getLiveObservableData() {
    ...
    public void setUiObservableData(FansData product) {
