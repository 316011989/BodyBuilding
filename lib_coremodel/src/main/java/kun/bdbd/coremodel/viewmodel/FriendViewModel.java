...
public class FriendViewModel extends AndroidViewModel {
    ...
    private LiveData<FriendData> mLiveObservableData;
    //UI使用可观察的数据 ObservableField是一个包装类，可使UI变更
    public ObservableField<FriendData> uiObservableData = new ObservableField<>();
    //后台调度，可关闭
    private final CompositeDisposable mDisposable = new CompositeDisposable();
    //动态数据，请求获取，可使mLiveObservableData变更
    private MutableLiveData<FriendData> applyData = new MutableLiveData<>();
    //动态boolean值，根据接口进度变化
    ...
    private void LoadData() {
        ...
        GankDataRepository.getFriendDataRepository(count, index)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<FriendData>() {
                    @Override
                    ...
                    @Override
                    public void onNext(FriendData value) {
                        LogUtils.e(value);
     ...
     * @return
     */
    public LiveData<FriendData> getLiveObservableData() {
    ...
    public void setUiObservableData(FriendData product) {
