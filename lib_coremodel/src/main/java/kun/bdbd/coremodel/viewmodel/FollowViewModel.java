...
public class FollowViewModel extends AndroidViewModel {
    ...
    private LiveData<FollowData> mLiveObservableData;
    //UI使用可观察的数据 ObservableField是一个包装类，可使UI变更
    public ObservableField<FollowData> uiObservableData = new ObservableField<>();
    //后台调度，可关闭
    private final CompositeDisposable mDisposable = new CompositeDisposable();
    //动态数据，请求获取，可使mLiveObservableData变更
    private MutableLiveData<FollowData> applyData = new MutableLiveData<>();
    //动态boolean值，根据接口进度变化
    ...
    private void LoadData() {
        ...
        GankDataRepository.getFollowDataRepository(count, index)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Observer<FollowData>() {
                    @Override
                    ...
                    @Override
                    public void onNext(FollowData value) {
                        LogUtils.e(value);
     ...
     * @return
     */
    public LiveData<FollowData> getLiveObservableData() {
    ...
    public void setUiObservableData(FollowData product) {
